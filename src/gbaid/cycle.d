    ...
    public class CycleSharer {
        ...
        public void takeCycles(uint id)(size_t cycles) if (id < numberOfSharers) {
            ...
            while (true) {
                ...
                enum barrierMask = id == 0 ? sharersMask : sharerBit;
                enum barrierCondition = id == 0 ? sharersMask : sharerBit;
                while (doneWithCycles.atomicOp!"&"(barrierMask) == barrierCondition) {
                    Thread.yield();
                    //writeln("1 ", id, ' ', doneWithCycles.atomicLoad!(MemoryOrder.raw), ' ', barrierCondition);
                }
