...
private void addAndSubtract(bool immediate, bool subtract)(Registers* registers, MemoryBus* memory, int instruction) {
    ...
    static if (subtract) {
        ...
        static if (__traits(compiles, SUB_WITH_FLAGS_ASM)) {
            int res = void;
            int flags = void;
            mixin (SUB_WITH_FLAGS_ASM);
            registers.setApsrFlagsPacked(flags);
        } else {
            int res = op1 - op2;
        ...
        static if (__traits(compiles, ADD_WITH_FLAGS_ASM)) {
            int res = void;
            int flags = void;
            mixin (ADD_WITH_FLAGS_ASM);
            registers.setApsrFlagsPacked(flags);
        } else {
            int res = op1 + op2;
